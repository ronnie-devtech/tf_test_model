...
对 tf_test_model 目录下所有包含 test_tf_musa_extension.py 的模型进行训练测试。
支持命令行指定训练轮数、指定GPU设备，每个模型的错误将保存到独立的日志文件中。

使用示例:
    python run_all_tests.py --epochs 10
    python run_all_tests.py --epochs 5 --log-dir error_logs
    python run_all_tests.py --epochs 10 --models deepfm wukong
    python run_all_tests.py --epochs 10 --gpu 0
    python run_all_tests.py --epochs 10 --gpu 0,1
"""
...
def run_model_test(
    model_name: str,
    epochs: int,
    tensorflow_musa_library_path: str,
    gpu_devices: str,
    log_dir: Path,
) -> Tuple[bool, str]:
    """运行单个模型的测试脚本
    ...
    Args:
        model_name: 模型名称
        epochs: 训练轮数
        tensorflow_musa_library_path: TensorFlow MUSA 库路径
    ...
    if tensorflow_musa_library_path:
        cmd.append(tensorflow_musa_library_path)
...
示例:
    python run_all_tests.py --epochs 10
    python run_all_tests.py --epochs 5 --log-dir error_logs
    python run_all_tests.py --epochs 10 --models deepfm wukong dien
    python run_all_tests.py --epochs 10 --gpu 0
    python run_all_tests.py --epochs 10 --gpu 0,1,2
    python run_all_tests.py --list-models
        """
    )
    ...
    for model_name in models_to_test:
        success, output = run_model_test(
            model_name,
            args.epochs,
            args.tensorflow_musa_library_path,
            args.gpu,
            log_dir,
        )
