...
LOSS_WEIGHT_INIT = [1.0, 1.0]
loss_weights = list(LOSS_WEIGHT_INIT)
...
####################################################################################################
#                                  HELPER: DERIVE TASK-2 LABEL                                    #
####################################################################################################
def get_task2_label(dense_inputs: tf.Tensor) -> tf.Tensor:
...
def train_step(inputs, labels):
    ...
    with tf.GradientTape() as tape:
        ...
        loss_task1 = task_criteria[0](labels_task1, tf.squeeze(logits_list[0]))
        loss_task2 = task_criteria[1](labels_task2, tf.squeeze(logits_list[1]))
        loss = loss_weights[0] * loss_task1 + loss_weights[1] * loss_task2
