...
# M=3 hierarchical field groups.
# Embedding tensor has shape (bs, NUM_CAT_FEATURES + NUM_DENSE_FEATURES, dim_emb).
# Indices 0-25: sparse features; 26-38: dense features.
_TOTAL_FEAT = NUM_CAT_FEATURES + NUM_DENSE_FEATURES  # 39
FIELD_GROUPS = [
    list(range(0, 9)),  # group 0: sparse indices 0-8
    list(range(9, 18)),  # group 1: sparse indices 9-17
    list(range(18, _TOTAL_FEAT)),  # group 2: sparse 18-25 + dense 26-38
]
...
model = FLEN(
    ...
    dicefactor_beta=DICEFACTOR_BETA,
    dropout=DROPOUT,
    use_bias=USE_BIAS,
)
...
_ = model((dummy_sparse, dummy_dense), training=False)
